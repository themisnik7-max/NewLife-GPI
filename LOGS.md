...
## 2026-07-25 — Feature: The Admin Write Layer (Create/Edit for Properties, Clients, and Procedures)

- **Type:** Feature + Bugfix
- **Summary:** Built the missing write half of the application. Until this entry, **this app was a viewer** — a full grep of `src/lib/data/*.ts` found exactly two real mutations in the entire codebase (`recordTenantPayment`, `revokeTenantApiKey`) plus one function built but never wired to any UI (`createTenantApiKey`). Nothing anywhere could create a Property, assign one to a client, add a construction milestone, add a visa step, or create a payment installment; every row that existed came from the Clerk webhook (users/tenants only) or a one-off manual seeding script. Added 7 new data-layer write functions, 2 admin-only editing panels, a property create/edit form, and 5 new Server Actions — all admin-gated. Verified: `tsc --noEmit` clean, 328/328 tests (29 files, up from 233/25), 100% coverage on every file touched, `next build` clean, and **every new write path independently verified against the live Postgres database**, not just mocks.

### Blocking prerequisite: the enum bug, exactly as predicted two entries ago
`PropertyStatus`, `RentalStage`, and `MilestoneStatus` were still Prisma `enum` blocks — the same bug class as `Role`/`PaymentStatus`/`ApiKeyStatus`, fixed two entries ago, and flagged there as "the moment any future code writes to these, it will fail exactly the same way." Every function in this entry does exactly that. Converted all three to `String` (no migration needed — `properties.status`, `property_ownerships.rental_stage`, `construction_milestones.status`, and `visa_steps.status` were already `text + check` in the migrations) and added explicit runtime validators wherever Prisma's compile-time narrowing was silently doing the work: `toFrontendStatus`'s new default-throw in `projects.ts`, new `toMilestoneStatus`/`toVisaStepStatus`, and applying the existing `toRentalStage()` to `getClientPropertySnapshot`'s previously-unvalidated pass-through. Only `AiLogStatus` remains a Prisma enum; nothing writes to `ai_logs` yet, so it stays flagged rather than pre-emptively changed.

### A wrong diagnosis I made and had to correct
Mid-session, live verification started failing with `ECONNREFUSED`. I chased it through raw `pg` (worked), the Prisma CLI (worked), and an explicitly-constructed pool (failed), matched it against two open Prisma issues (#28739, #27626), upgraded `prisma`/`@prisma/client`/`@prisma/adapter-pg` 7.8.0 → 7.9.0 attempting a real fix, and **reported it to the user as a confirmed upstream bug possibly affecting production**. That was wrong. The actual cause: `DATABASE_URL` is simply not loaded inside Vitest, so `PrismaPg` received `undefined` and pg fell back to its default of `localhost:5432` — a genuine connection refusal from this machine. `factory.connect()` appeared to succeed only because pg pools connect lazily. **The app itself was never affected** (Next.js loads `.env` natively); it was purely a throwaway test file missing `dotenv`. Fixed by loading dotenv explicitly in the live-verification file — deliberately NOT in `vitest.config.ts`'s global setup, since every real unit test mocks `@/lib/prisma` outright and the suite must never carry live credentials that could let a stray test write to the real database. The 7.9.0 upgrade was kept (it's a clean patch bump that changes nothing here). Lesson recorded plainly: "raw driver works, ORM doesn't" pointed at the library, but the far cheaper check — *is the connection string even defined?* — would have found it in one step.

### What was built
- **Properties (`src/lib/data/projects.ts`):** `createProperty`/`updateProperty`. `imageUrl`/`mapUrl` fall back to the same derivations `MOCK_PROJECTS` already used (`buildPlaceholderImageUrl`/`buildMapUrl`, now exported) rather than making an admin hand-type a Maps link or source a photo for a field with no upload mechanism behind it. `updateProperty` fetches the existing row first specifically so the `availableUnits <= totalUnits` invariant can be validated on a *partial* update — decrementing `availableUnits` alone, the most realistic edit, needs the row's stored `totalUnits` to check against.
- **Clients (`propertyOwnership.ts`):** `assignPropertyToClient` (verifies both property and user belong to the tenant; idempotent, since `@@unique([userId, propertyId])` would otherwise surface a raw constraint error) and `updateRentalStage` (targets the most recent ownership, matching the read path's own `orderBy: createdAt desc` — otherwise an admin could update a stage no screen displays).
- **Construction (`construction.ts`):** `createMilestone`/`updateMilestoneStatus`, auto-stamping `completionDate` on COMPLETED and clearing it when moved back off, so a row can't claim a completion date for unfinished work. Lives on the property detail page, not the client page — a milestone describes the *property's* build progress, per its own schema doc comment.
- **Visa (`visa.ts`):** `createVisaStep`/`updateVisaStepStatus`. `stepOrder` auto-assigns as "next after the client's current highest" rather than requiring hand-tracked numbering against a `@@unique([userId, stepOrder])` constraint; an explicit value is still honored for out-of-sequence inserts.
- **Payments (`ledgers.ts`):** `createLedgerEntry` — the missing counterpart to `recordTenantPayment`, which could only pay against a row that already existed.
- **UI:** `PropertyForm` (one form, both create and edit), `/dashboard/projects/new`, an inline admin Edit toggle on property detail, `ClientAdminPanel` (assign property, advance rental stage, add/update visa steps, add installments), `MilestoneAdminPanel`, and shared `adminControls.tsx` primitives so the two panels can't drift apart on pending/error handling. Every panel is rendered *additively* alongside the existing read-only components rather than modifying them — `ClientOverviewSummary`, `ConstructionMilestones`, and `ProjectDetail` are all still used unchanged on client-facing pages.
- **Discoverability:** an "Invite Client" link from the admin Overview to `/dashboard/team`, pointing at the Clerk `<OrganizationProfile />` invite flow built last entry — it already existed, it just wasn't reachable from where an admin would look. Deliberately not a second, parallel add-client mechanism.

### Security posture
Every one of the 5 new Server Actions independently re-checks `currentUser.role === Role.ADMIN` server-side via `getCurrentUser()` — never trusting that a control was merely hidden client-side. `tenantId` always comes from the signed-in admin's own resolved session, never from a parameter, which is what stops a crafted request naming a `userId`/`propertyId` in another tenant from writing across the boundary. This matters more than usual here because Prisma bypasses RLS entirely (ARCHITECTURE.md), so these application-level checks *are* the boundary.

### Verification
`npx tsc --noEmit` clean → `npm run test` 328/328 (29 files, up from 233/25) → per-file coverage 100%/100%/100%/100% on all 9 files touched or created (checked with single-file runs: Vitest's v8 provider merges coverage incorrectly across worker threads when multiple files run together — a known upstream limitation, vitest-dev/vitest#5783 — so the aggregate run under-reports files a second test file never touched) → `npm run build` clean, `/dashboard/projects/new` correctly `ƒ` dynamic → **live database round-trip of every new write path**: property create+status update, ownership assign (twice, proving idempotency) + rental stage → HANDOVER, milestone create + COMPLETED with auto `completionDate`, visa step create with auto `stepOrder` + status update, ledger entry create, then full cleanup verified. That single test exercised all three newly-converted enum columns against real Postgres, which is the only way this class of bug is catchable.

- **Files touched:** `prisma/schema.prisma` (3 enums → String), `src/lib/data/projects.ts`, `propertyOwnership.ts`, `construction.ts`, `visa.ts`, `ledgers.ts` (7 new write functions + validators), `src/lib/projects.ts` (2 helpers exported), `src/components/ui/PropertyForm.tsx` + `adminControls.tsx` (created), `src/app/dashboard/projects/actions.ts` + `clients/[userId]/actions.ts` (created), `projects/new/page.tsx` + `PropertyFormClient.tsx` (created), `projects/[id]/PropertyDetailWithEdit.tsx` + `MilestoneAdminPanel.tsx` (created), `clients/[userId]/ClientAdminPanel.tsx` (created), 4 pages wired, 6 test files extended/created, `package.json` (Prisma 7.9.0), `LOGS.md`.
- **Status:** Complete and verified for what was asked. Deliberately scoped out, flagged rather than silently skipped: **delete** operations for any entity (deleting a property cascades into milestones/ownerships/ledger entries — materially riskier than the create/edit that was asked for, and worth its own deliberate design); `AiLogStatus` remains the last Prisma enum of this bug class (unreachable today, nothing writes `ai_logs`); the Clerk development-instance issue from the previous entry is still unresolved and still blocks confirming the deployed app in a browser.

## 2026-07-27 — Bugfix: Live Browser Testing, and Removing the Supabase JS Client Entirely

- **Type:** Bugfix + Architecture simplification
- **Summary:** The user reported "I tried property and it showed error" and asked for a real live test of both accounts rather than another file-reading pass. Set up Playwright driving real Chrome, authenticated both accounts through Clerk's sign-in-token mechanism, and walked every route on **both localhost and production**. Found three pages returning HTTP 500 — the only three that read through `@supabase/supabase-js`. Removed that entire code path; the app now reaches Postgres exclusively through Prisma. Verified live on production afterwards. 301/301 tests, 100% coverage on the rewritten module, clean build.

### Two distinct bugs, one symptom
1. **`supabaseUrl is required`** (the error the user actually saw locally): `NEXT_PUBLIC_SUPABASE_URL`/`ANON_KEY` had been set in Vercel during the deploy session but never added to local `.env.local`. Adding them locally fixed that error and immediately exposed the real one underneath.
2. **`No suitable key or wrong key type`** (the real bug, identical on localhost *and* production): Supabase was rejecting the Clerk JWT because the Clerk ↔ Supabase Third-Party Auth bridge was never configured. `ENVIRONMENT_DEPLOY.md` §2 had documented it as a manual two-dashboard step; it was written but never performed. So `/dashboard/property`, `/dashboard/construction`, and `/dashboard/rental` had **never worked in production at all**.

### The fix, and why this direction
Given the user's stated goal — a 24/7 database with all data centralized for later analysis — the choice was between completing the JWT bridge or deleting the path that needed it. Deleted it: ~90% of the app already used Prisma, and the Supabase client was reached by exactly two functions. `getOwnedProperty()` and `getCurrentRentalStage()` are gone, replaced by the already-existing `getClientPropertySnapshot()`; `src/lib/supabaseClient.ts` and its test are deleted; `@supabase/supabase-js` is uninstalled; the two env vars are removed from `.env.local`, `.env.example`, and Vercel. **Supabase remains the database** — only its JS client is gone.

### The security detail that mattered most
`getOwnedProperty(token, tenantId)` took only a tenantId and leaned on RLS to add `user_id = app.current_clerk_user_id()` **implicitly**. Prisma bypasses RLS, so a naive port would have silently returned whichever tenant member's ownership row was most recent — and in this deployment the admin and demo client deliberately share one tenant, so the admin's own "My Property" page would have shown the client's Villa Elytra. The replacement takes `userId` explicitly. Verified live, not assumed: after the change the admin correctly sees "No property is currently assigned to your account" on all four per-user pages while the client sees their real data, on both localhost and production.

Consequence stated plainly in `ARCHITECTURE.md`: **RLS is no longer an enforcement boundary anywhere in this app.** It was already bypassed by ~90% of the code; it is now bypassed by 100%. Tenant isolation is entirely application-level. The RLS policies stay in the migrations — harmless, still correct, and live again if anything ever reads this database through PostgREST — but they are not what protects the app today. `ENVIRONMENT_DEPLOY.md`'s post-deploy checklist was rewritten to verify *that* reality (cross-role isolation checks) instead of the removed JWT bridge.

### What the live test actually proved
Beyond the bug: sign-in works on production (an earlier entry had flagged a suspected Clerk dev-instance problem — that concern was unfounded, and is now retired); role separation is genuinely enforced, not cosmetic (a client requesting `/dashboard/team` or `/dashboard/projects/new` by URL gets a hard 404); and **every admin write control built in the previous entry works end-to-end in a real browser** — create property, inline edit, add milestone + change status, add visa step, add installment, update rental stage. Two of the thirteen browser assertions initially "failed"; both turned out to be my own selectors being wrong (`getByLabel("Address")` substring-matched the Map URL helper text), not app faults — screenshots confirmed the underlying features worked. All test data injected into the demo account was removed afterwards and the pre-test state restored exactly (5 visa steps, 3 installments, €50,000 outstanding, rental stage back to Vendors Engaged).

- **Files touched:** `src/lib/data/propertyOwnership.ts` (Supabase path removed), `src/lib/supabaseClient.ts` + `.test.ts` (deleted), `src/app/dashboard/{property,construction,rental}/page.tsx`, `src/lib/data/propertyOwnership.test.ts` (Supabase suites dropped), `ARCHITECTURE.md` (JWT-bridge section replaced), `ENVIRONMENT_DEPLOY.md`, `.env.example`, `package.json` (`@supabase/supabase-js` removed, `playwright` added as a devDependency for this kind of testing), `LOGS.md`.
- **Status:** Complete and verified on production. Open items unchanged: `AiLogStatus` is the last Prisma enum of the known bad class (unreachable — nothing writes `ai_logs` yet); delete operations still deliberately unbuilt; and `ai_logs`/`EncryptedApiKey` remain defined-but-unwritten, which matters for the stated data-analysis goal — flagged to the user rather than silently built.

## 2026-07-27 — Feature: Append-Only Audit Trail, and a Rewritten Rental Workflow with File Attachments

- **Type:** Feature
- **Summary:** Two requests off the back of the data-analysis goal: build the audit trail flagged last entry as the expensive, unbackfillable gap, and replace the rental stages with the real letting workflow. Both done, plus Supabase Storage for the document slots. Verified: `tsc --noEmit` clean, 367/367 tests (31 files, up from 296/27), 100% coverage on all four new modules, `next build` clean, a **live Postgres round-trip** of every new write, and a **live browser test** of both accounts. All injected test data was removed afterwards.

### Part 1 — Audit trail
New append-only `AuditLog` (`audit_logs`), shaped after `AiLog` (no `updatedAt`, `Json?` metadata) but deliberately **not** copying its `status` field — `AiLogStatus` is this schema's one real Prisma `enum` and is the exact bug class documented on `User.role`. Everything here is `String`.

`recordAuditEvent(writer, …)` takes the writer as its first argument specifically so callers hand it the `tx` from their own `prisma.$transaction`: the audit row and the change it describes commit or roll back together. A mutation that succeeded while its audit row silently failed is precisely the gap this feature exists to close, so it is never fire-and-forget. Wired into **all ten** existing mutations (`createProperty`/`updateProperty`, `assignPropertyToClient`, `createMilestone`/`updateMilestoneStatus`, `createVisaStep`/`updateVisaStepStatus`, `createLedgerEntry`/`recordTenantPayment`, `createTenantApiKey`/`revokeTenantApiKey`), each now transactional; `recordTenantPayment` extends its existing transaction rather than opening a second.

Three decisions worth recording:
- **`ActorContext` object, not two positional strings.** Every audited mutation's first parameter changed from `tenantId: string` to `{ tenantId, actorUserId }`. Several of these already take multiple adjacent id strings (`assignPropertyToClient(tenantId, userId, propertyId)`), and transposing an actor id with a subject id would be a security bug that still compiles.
- **`recordFieldChanges` skips no-op writes.** Setting a status to the value it already had produces no row. Recording it would put phantom events in the timeline and distort exactly the "when did this actually happen" analysis the table exists to support — verified live that a repeat write adds nothing.
- **Null is preserved as SQL NULL**, not the strings `"null"`/`"undefined"`, which would be indistinguishable from real values during analysis. API-key auditing records provider/label/maskedKey only — never raw key material or ciphertext; an audit trail that leaks the secret it audits is worse than none.

Scoped out and flagged: **no audit-viewer UI.** The table is the deliverable and is queryable directly (`getAuditTrail()`); a UI is separate, larger work.

### Part 2 — The rental workflow, rebuilt
The old model was a single `PropertyOwnership.rentalStage` scalar holding one of ten *purchase* states, rendered as "everything before the current index is done". The requested process is a *letting* workflow: ten stages, each with its own DONE/PENDING status, five carrying a document, one carrying structured offer terms. A scalar cannot express any of that.

`property_ownerships.rental_stage` is **dropped** (migration `0008`). No backfill: the old vocabulary (RESERVATION…RENTAL_ACTIVE) has no honest mapping onto the new one (mandate → inspection → keys → EPC → electricity → photo/video → viewings → offer → contract → broker's fee), so inventing one would fabricate history rather than preserve it. Concretely this discarded one demo row's `VENDORS_ENGAGED`.

**No row means PENDING.** The canonical ten stages live in code (`src/lib/rentalStages.ts`); `rental_stage_records` stores a row only once a stage is actually touched. This avoids a backfill, keeps ten rows-per-client of "nothing has happened" out of the table, and means revising the stage list later cannot strand orphaned rows — a row whose `stageKey` is no longer canonical is ignored on read rather than throwing, so a retired stage can't break every page. `stage_key` deliberately has **no** check constraint for the same reason; it is validated in application code against the canonical list on both read and write.

Offer duration is an **integer month count**, not free text — chosen explicitly so "how long are the leases we actually agree?" stays answerable without parsing prose. Offer fields are nullable columns on the shared table rather than a separate one-row table; one stage uses them and a dedicated model for three fields would add a join for no gain.

`RentalRoadmap` and its test are deleted, replaced by `RentalStageTracker`. `getClientPropertySnapshot` no longer carries `rentalStage`; all five call sites moved together.

### Part 3 — File attachments (Supabase Storage)
Re-adds `@supabase/supabase-js`, **which I removed four days ago** — stated plainly rather than slipped in. It is a different thing for a different reason: Storage only, server-side only, authenticated with a `sb_secret_…` key, and needing none of the Clerk↔Supabase JWT bridge that never worked. Every database read and write still goes through Prisma exclusively; the PostgREST data path is not back.

Bucket `rental-documents` is private; downloads are served through 60-second signed URLs minted server-side, and the stored object path is never sent to the browser. Content type is validated against the slot's allowlist server-side — the browser's `accept` attribute is a convenience, not a control. Uploads use `upsert: false` so re-uploading keeps the object an earlier audit row refers to. **Degrades gracefully:** with no key set, slots render "File storage is not configured" and every other part of the page keeps working — which is the state the live browser test actually exercised, since the key is the user's to supply.

Slots are assigned exactly as specified (4 PDF, 1 photo). `PROFESSIONAL_PHOTO_VIDEO` plausibly wants one but none was requested, so it has none; the mechanism is generic, so adding it later is a one-word change.

### Verification
`npx tsc --noEmit` clean → `npm run test` 367/367 (31 files, up from 296/27) → per-file coverage 100%/100%/100%/100% on `audit.ts`, `rentalStages.ts` (both), `storage.ts`, `RentalStageTracker.tsx` → `npm run build` clean → **live Postgres round-trip**: ten stages read with nothing stored, DONE stamps `completedAt`, reverting clears it, offer `Decimal(1450.75)`/`Int(24)` round-trip exactly, five audit rows landed with correct before/after values and actor, and a repeat no-op write added none → **live browser test** (Playwright/real Chrome, both accounts): admin sees all ten new stages in order with the old purchase stages gone, marking DONE persists and the Overview card updates to "1 of 10 stages complete", offer terms persist, and the client sees the admin's changes with zero admin controls and zero `<select>` elements. One of 22 browser assertions "failed" — my own case-sensitive check against a CSS-uppercased heading, confirmed against the screenshot, not an app fault.

- **Files touched:** `prisma/schema.prisma` (`AuditLog` + `RentalStageRecord` added, `PropertyOwnership.rentalStage` removed), `supabase/migrations/0008_audit_log_and_rental_stages.sql` (created, applied live), `src/lib/data/audit.ts` + `rentalStages.ts` + `src/lib/rentalStages.ts` + `src/lib/storage.ts` (created), all six existing `src/lib/data/*.ts` mutation modules (audited + `ActorContext`), `src/components/ui/RentalStageTracker.tsx` (created), `RentalRoadmap.tsx` + its test (deleted), `ClientOverviewSummary.tsx`, `ClientAdminPanel.tsx`, both actions files, four pages, four new test suites plus six updated, `.env.example`, `ENVIRONMENT_DEPLOY.md`, `LOGS.md`.
